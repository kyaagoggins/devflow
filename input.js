...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (savedData) {
    ...
    form.frontend_team.value = data.team?.teamType?.["Frontend"]?.join(', ') || '';
    form.backend_team.value = data.team?.teamType?.["Backend"]?.join(', ') || '';
    ...
    form.next_month_deadlines.value = data.deadlines?.nextMonth?.join('\n') || '';
    form.risks.value = Array.isArray(data.risks) ? data.risks.join('\n') : (data.risks || '');
    form.requirements.value = Array.isArray(data.requirements) ? data.requirements.join('\n') : (data.requirements || '');
  ...
  function addTaskItem(task = {}) {
    ...
    newTask.innerHTML = `
      <label>Task Information:
        <input type="text" name="task_info[]" required value="${task.taskInfo ? escapeHtml(task.taskInfo) : ''}">
      </label>
      <label>Due Date:
        <input type="date" name="task_due_date[]" required value="${task.dueDate ? escapeHtml(task.dueDate) : ''}">
      </label>
      <label>Priority:
      ...
      <label>Team Assigned:
        <select name="task_team[]" required>
          <option value="UI/UX" ${task.teamAssigned === 'UI/UX' ? 'selected' : ''}>UI/UX</option>
          <option value="Frontend" ${task.teamAssigned === 'Frontend' ? 'selected' : ''}>Frontend</option>
          <option value="Backend" ${task.teamAssigned === 'Backend' ? 'selected' : ''}>Backend</option>
          <option value="QA" ${task.teamAssigned === 'QA' ? 'selected' : ''}>QA</option>
          <option value="All" ${task.teamAssigned === 'All' ? 'selected' : ''}>All</option>
        </select>
      </label>
